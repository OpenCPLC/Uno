...
char *cut_this(char *str, char *pattern, EXSTRING_Cut_e mode)
{
  int x;
  switch(mode) {
    case EXSTRING_Cut_StartLeft_GetLeft:
      x = find(str, pattern);
      if(x) str[find(str, pattern) - strlen(pattern)] = 0;
      break;
    case EXSTRING_Cut_StartRight_GetLeft:
      x = find_right(str, pattern);
      if(x) str[strlen(str) - x] = 0;
      break;
    case EXSTRING_Cut_StartLeft_GetRight:
      x = find(str, pattern);
      if(x) str = &str[x];
      break;
    case EXSTRING_Cut_StartRight_GetRight:
      x = find_right(str, pattern);
...
char *cut(char *str_in, char *pattern, EXSTRING_Cut_e mode)
{
  size_t size = strlen(str_in);
  char *str_out = new(size + 1);
  memcpy(str_out, str_in, size + 1);
 ...
 */
char *replace_str(char *pattern, char *replacement, char *original)
{
