 ...
 * Zeruje wartości wypełnienia `fill` dla wszystkich kanałów PWM.
...
/**
 * @brief Ustawia wartość wypełnienia `fill` sygnału PWM na wyjściu cyfrowym.
 ...
 * @param fill Wartość wypełnienia [%] sygnału PWM (0% - 100%).
 * @return Rzeczywista ustawiona wartość wypełnienia `fill` na wyjściu cyfrowym.
 */
float DOUT_Fill(DOUT_t *dout, float fill)
{
  uint32_t old_value = dout->pwm->value[dout->channel];
  PWM_SetValue(dout->pwm, dout->channel, fill * dout->pwm->auto_reload / 100);
  dout->value = dout->pwm->value[dout->channel];
  float fill = (float)dout->value * 100 / dout->pwm->auto_reload;
  if((old_value != dout->value) && dout->save) EEPROM_Write(&dout->eeprom, &dout->value);
  return fill;
}
...
void DOUT_Set(DOUT_t *dout)
{
  ...
  if(dout->save) EEPROM_Write(&dout->eeprom, &dout->value);
}
...
void DOUT_Init(DOUT_t *dout)
{
  ...
  EEPROM_Read(&dout->eeprom, &dout->save);
